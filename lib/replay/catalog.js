...
parseHeaders = function(filename, header_lines, only) {
  var flags, headers, in_regexp, key, line, name, regexpBody, value, _, _i, _len, _ref, _ref1;
  if (only == null) {
    only = null;
  }
  ...
  for (_i = 0, _len = header_lines.length; _i < _len; _i++) {
    ...
    if (/REGEXP\s/.test(value)) {
      regexpBody = value.split("REGEXP ")[1];
      _ref1 = regexpBody.match(/^\/(.+)\/(i|m|g)?$/), _ = _ref1[0], in_regexp = _ref1[1], flags = _ref1[2];
