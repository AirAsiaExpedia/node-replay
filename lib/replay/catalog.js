...
var Catalog, File, Matcher, Path, assert, exists, existsSync, isBinary, jsStringEscape, match, mkdir, parseHeaders, writeHeaders,
  __slice = [].slice;
...
Catalog = (function() {
  ...
  Catalog.prototype._read = function(filename) {
    ...
    parse_request = function(request) {
      var body, flags, header_lines, headers, in_regexp, method, method_and_path, path, raw_regexp, regexp, regexpBody, _, _ref, _ref1, _ref2, _ref3, _ref4;
      ...
      if (/REGEXP\s/.test(body)) {
        ...
        _ref4 = regexpBody.match(/^\/(.+)\/(i|m|g)?$/), _ = _ref4[0], in_regexp = _ref4[1], flags = _ref4[2];
...
parseHeaders = function(filename, header_lines, only) {
  var flags, headers, in_regexp, key, line, name, regexpHeader, value, _, _i, _len, _ref, _ref1;
  if (only == null) {
    only = null;
  }
  ...
  for (_i = 0, _len = header_lines.length; _i < _len; _i++) {
    ...
    if (/REGEXP\s/.test(value)) {
      ...
      _ref1 = regexpHeader.match(/^\/(.+)\/(i|m|g)?$/), _ = _ref1[0], in_regexp = _ref1[1], flags = _ref1[2];
...
isBinary = function(headers) {
  return headers['content-type'] === 'application/octet-stream' || /(gzip|deflate)/.test(headers['content-encoding']);
};
